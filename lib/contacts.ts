...
/**
 * Shape one uploaded row into dialer field names without dropping it.
 * Does not require a DB match — used by the CSV Format page.
 */
export function shapeDialerCsvRow(row: CsvRecord): CsvRecord {
  ...
  const timezone =
    cell(row, 'Time_Zone', 'Time Zone', 'Timezone') || '';
  const specialtyOrTax = cell(row, 'taxonomy', 'Taxonomy', 'Specialty', 'Taxonomy Code');
  const codeOnly = cell(row, 'Taxonomy Code');
  const taxonomyCode =
    (specialtyOrTax.match(/\(([A-Z0-9]+)\)\s*$/i) || [])[1] ||
    codeOnly ||
    (/^[A-Z0-9]+X$/i.test(specialtyOrTax.trim()) ? specialtyOrTax.trim() : '') ||
    '';

  next['NPI'] = cell(row, 'NPI', 'NPPES Number', 'NPPES', 'npi');
  next['Practice_Name'] = practiceName;
  next['Authorized_Person'] = person;
  next['Authorized_Title'] = normalizeDecisionMakerTitle(titleRaw);
  next['Phone'] = cell(row, 'Phone', 'Practice Phone') || '';
  next['Address'] = cell(row, 'Address') || '';
  next['City'] = cell(row, 'City') || '';
  next['State'] = cell(row, 'State') || '';
  next['ZIP'] = cell(row, 'ZIP', 'Zip', 'Postal Code') || '';
  next['taxonomy'] =
    formatTaxonomyCell(taxonomyCode) ||
    (specialtyOrTax.includes('(') ? specialtyOrTax : specialtyOrTax) ||
    '';
  next['Enumeration_Date'] = cell(row, 'Enumeration_Date', 'Enumeration Date') || '';
  next['Time_Zone'] = timezone;
  next['PKT_Call_Window'] =
    cell(row, 'PKT_Call_Window', 'PKT Call Window') || pktCallWindow(timezone);

  if (!next['Providers']) next['Providers'] = cell(row, 'Providers', 'Provider Count') || '';
  if (!next['Branches']) next['Branches'] = cell(row, 'Branches', 'Branch Count', 'Locations') || '';

  return next;
}

/**
 * Format-only: rewrite dialer column order + aliases for every row.
 * Never drops rows for size / DB unmatched (unlike filterUploadedContactsCsv).
 */
export function formatUploadedContactsCsv(
  csvText: string,
  opts: { merge?: boolean } = {}
): {
  csv: string;
  ...
  const shaped = rows.map(shapeDialerCsvRow);
  const merge = opts.merge !== false;
  const finalRows = merge ? mergeDialerCsvRows(shaped) : shaped;
  return {
    csv: buildDialerCsvFromRecords(finalRows),
    rows: finalRows,
    total: rows.length,
    kept: finalRows.length,
    mergedAway: Math.max(0, shaped.length - finalRows.length),
  };
}
